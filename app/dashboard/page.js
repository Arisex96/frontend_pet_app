...
const Dashboard = () => {
  ...
  const [pets, setPets] = useState([]);  // State to store public pets
  const slides = [
  ...
  // Fetch public pets from the API when the component mounts
  useEffect(() => {
    const fetchPets = async () => {
      const response = await fetch('http://localhost:8000/api/auth/dashboard/pets/');
      if (response.ok) {
        const data = await response.json();
        setPets(data);  // Update the state with fetched pets
      } else {
            ...
            pets.map((pet, index) => (
              ...
              >
                <img
                  ...
                  src={`http://localhost:8000${pet.photos}`}  // Use the pet's photo URL
                  alt={pet.name}
                  width={400}
                  height={200}
                />
